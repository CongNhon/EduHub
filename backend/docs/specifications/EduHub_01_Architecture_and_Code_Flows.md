...
### 20.8 Two-repository Docker topology

```text
EduHub.Backend -> API + migrate one-shot + optional seed one-shot
               -> PostgreSQL + Redis + MongoDB + named volumes
EduHub.Frontend -> Site + Portal
Both repositories -> external Docker network eduhub-dev
Portal BFF -> http://eduhub-api:8080
Browser -> http://localhost:3000 / http://localhost:3001
```

- Backend `.gitignore` loại toàn bộ `/frontend`; thư mục `frontend` có Git metadata, CI và Docker riêng.
