...
/// <summary>
/// Ghi chú: WeeklyDigestJob gửi fake email tổng hợp tuần cho phụ huynh có grade Published/Locked.
/// </summary>
...
{
    private const string TemplateVersion = "weekly-digest-v1";

    /// <summary>
    /// Ghi chú: SendWeeklyDigestAsync gửi fake email theo recipient/period/template version, không chứa Draft grade.
    /// </summary>
    public async Task SendWeeklyDigestAsync(CancellationToken cancellationToken = default)
    {
        var now = timeProvider.GetUtcNow().UtcDateTime;
        var periodStart = now.Date.AddDays(-7);
        ...
        foreach (var recipient in recipients)
        {
            ...
            try
            {
                var email = GradeDigestEmailTemplate.Render("weekly", periodStart, periodEnd, items);
    ...
    /// <summary>
    /// Ghi chú: GetDigestItemsAsync lấy danh sách điểm Published/Locked của các học sinh thuộc một phụ huynh trong tuần.
    /// </summary>
